    ...
    return Scaffold(
      ...
      body: BlocListener<GetHijriDateCubit, GetHijriDateState>(
        listener: (context, state) {
          if (state is GetHijriDateLoaded) _refreshController.refreshCompleted();
          if (state is GetHijriDateError) _refreshController.refreshFailed();
        },
        child: BlocBuilder<BottomNavigationBarCubit, int>(
          builder: (context, state) {
            return SmartRefresher(
              enablePullDown: true,
              onRefresh: () => context.read<GetHijriDateCubit>().getTodayDate(),
          ...
          return Container(
            ...
            child: BottomNavigationBar(
              ...
              items: const [
                ...
                BottomNavigationBarItem(icon: Icon(Icons.diamond_rounded), activeIcon: Icon(Icons.diamond_rounded), label: 'Tasbih'),
                BottomNavigationBarItem(icon: Icon(Icons.newspaper_rounded), activeIcon: Icon(Icons.newspaper_rounded), label: 'History'),
              ],
            ),
          );
        },
