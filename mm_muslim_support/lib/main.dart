...
void main() async {
  ...
  if(SharedPreferenceService.getFirstOpen() ?? true) {
    await SharedPreferenceService.setFirstOpen(false);
    await SharedPreferenceService.setHijriOffset(-2);
  }
...
Future<void> getLocationFromDevice() async {
  try {
    // Attempt to get location using Geolocator OR Native Fallback channel.
    bool success = await getLocation();
    
    if (success) {
      return; // Successfully got location and saved it.
    }
  } catch (_) {}
  
  // Ultimate Fallback: if all location providers (Google + Native) fail, default to Yangon
